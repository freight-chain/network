...
module.exports = function (app) {
    ...
    const messages = {
        ...
        TX_HAS_BEEN_MINED_WITH_FALSE_STATUS:
            'Transaction has been mined, but status is false',
    ...
    app.post('/', async function (request, response) {
        ...
        if (
            await validateCaptchaResponse(captchaResponse, receiver, response)
        ) {
    ...
    async function validateCaptchaResponse(
        captchaResponse,
        receiver,
        response
    ) {
    ...
    async function sendPOAToRecipient(web3, receiver, response, isDebug) {
        ...
        const ethToSend = web3.utils.toWei(
